...
def evaluate_and_plot(model, df_daily, target_col, test_loader, target_scaler, y_test_orig, model_name):
    model.eval()
    y_pred_all = []
    ...
    plt.title(f"{target_col} Multivariate Forecast vs Reality ({model_name})", fontweight="bold")
    plt.xlabel("Date")
    plt.ylabel(f"{target_col} Level")
    plt.legend(loc="upper left")
    plt.grid(True, linestyle="--", alpha=0.4)
    plt.tight_layout()
    plt.savefig(f"outputs/plots/{target_col}_{model_name}_multivariate_forecast.png", dpi=150)
    plt.show()
...
def main_execution() -> None:
    ...
    for target in targets:
        ...
        metrics_lstm = evaluate_and_plot(lstm_model, df_daily, target, test_loader, scaler, y_test_orig, model_name="LSTM")
        lstm_results[target] = metrics_lstm
        ...
        metrics_bilstm = evaluate_and_plot(bilstm_model, df_daily, target, test_loader, scaler, y_test_orig, model_name="Bi-LSTM")
        bilstm_results[target] = metrics_bilstm

        print(f"\n [✓] LSTM    Metrics: RMSE={metrics_lstm['RMSE']:.3f}, MAE={metrics_lstm['MAE']:.3f}, MAPE={metrics_lstm['MAPE']:.2f}%")
        print(f" [✓] Bi-LSTM Metrics: RMSE={metrics_bilstm['RMSE']:.3f}, MAE={metrics_bilstm['MAE']:.3f}, MAPE={metrics_bilstm['MAPE']:.2f}%")
