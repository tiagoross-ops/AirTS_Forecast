...
File: environmental_data_exp_timeseries.py
...
Includes Parquet data exportation for downstream ML pipelines.
"""
...
from environmental_data_exp_retrieval import (file_name_comprehension, period_retrieval_function)
...
def export_timeseries_to_parquet(
        continuous_data_dict: dict[str, pd.DataFrame],
        output_dir: str | Path = "Exported_Parquet_Data"
) -> list[Path]:
    """
    Exports the continuous environmental timeseries DataFrames to Parquet format.
    Collapses all spatial dimensions to generate a single-column DataFrame representing
    the overall hourly average for the entire region, optimizing it for 1D RNN ingestion.

    Args:
        continuous_data_dict (dict): Dictionary mapping variables to their native time-series DataFrames.
        output_dir (str | Path): Directory to store the resulting .parquet files.

    Returns:
        list[Path]: List of absolute paths to the generated Parquet files.
    """
    target_dir = Path(output_dir)
    target_dir.mkdir(parents=True, exist_ok=True)
    exported_paths = []

    if not continuous_data_dict:
        logger.warning("No data provided to the Parquet exporter.")
        return exported_paths

    for var, native_df in continuous_data_dict.items():
        try:
            # Calculate the overall regional average per timestamp (row-wise mean)
            # This collapses all Lat/Lon columns into a single 1D time-series
            regional_avg_series = native_df.mean(axis=1)

            # Convert to a DataFrame with exactly one column named after the variable
            single_col_df = pd.DataFrame({var: regional_avg_series})

            # Ensure the index is explicitly named (standardizing for downstream merges)
            single_col_df.index.name = "Timestamp"

            output_path = target_dir / f"{var}_environmental_timeseries.parquet"

            # Write to Parquet using PyArrow with Snappy compression
            single_col_df.to_parquet(output_path, engine="pyarrow", compression="snappy")
            logger.info(f"Successfully exported overall 1D {var.upper()} average timeseries to {output_path.name}")

            exported_paths.append(output_path)

        except ImportError:
            logger.error("Missing dependency. Please ensure 'pyarrow' is installed to write Parquet files.")
            break
        except Exception as e:
            logger.error(f"Failed to export {var} to Parquet: {e}")
...
if __name__ == '__main__':
    target_dir = Path("Analysis - 02 round/era5_monthly_data")
    ...
    if target_dir.exists():
        ...
        try:
            ...
            # 4. Export Continuous Timeseries to Parquet for ML pipelines
            logger.info("\nExecuting Parquet Export Pipeline...")
            export_timeseries_to_parquet(
                continuous_overall_data,
                output_dir="Exported_Parquet_Data"
            )

            # 5. PDF APPLICATION OF THE MASTER ORCHESTRATOR
